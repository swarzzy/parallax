...
namespace prx {
	...
	Renderable2D::Renderable2D()
		: m_Position(hpm::vec3(0.0)), m_Size(hpm::vec2(0.0)), 
		m_Color(0xffffffff), m_Texture(nullptr), m_Reflected(false) 
	{ setDefaultUVs(); }

	prx::Renderable2D::Renderable2D(const hpm::vec3& position, const hpm::vec2& size, unsigned int color, bool reflected)
		: m_Position(position), m_Size(size), m_Color(color), m_Texture(nullptr), m_Reflected(reflected)
	{ setDefaultUVs(); }

	Renderable2D::Renderable2D(const hpm::vec3& position, const hpm::vec2& size, TextureBase* texture, bool reflected)
